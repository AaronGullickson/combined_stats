...
plot(ha$povrate, ha$amtcapita, 
     ...
     pch=21, bg=rainbow(5)[ha$borough], col="black", cex=log(ha$popn/3000,5), 
     bty="n")
...
lg <- legend(0, 100000, 
             ...
             pch=21, col="black", pt.bg=rainbow(5), ncol=5,
...
The final step is to color-code the dots. Luckily, the borough codes already go from 1 to 5. I can utilize this to simplify my task. If I create a vector of color names of size 5, I can then use the borough codes to reference these colors by the index of the vector. Lets choose some unusual colors:

```{r pretty12}
colorChoices <- c("wheat3", "steelblue2","seashell","salmon","navajowhite")
par(mar=c(5,6,3,1))
plot(ha$povrate, ha$amtcapita, 
     log="y", yaxt="n", ylim=c(0.1,100000), ylab="",
     xaxt="n", xlim=c(0,60), xlab="Poverty rate",
     pch=21, bg=colorChoices[ha$borough], col="black", cex=log(ha$popn/3000,5))
axis(2,at=c(0.1,1,10,100,1000,10000,100000), las=2,
     labels=paste("$",formatC(c(0.1,1,10,100,1000,10000,100000), 
                                  format="d", big.mark=","), sep=""))
title(ylab="amount per capita", line=5)
...
That worked out pretty decently, but there are also a couple of useful functions such as `rainbow`, `heat.colors`, `terrain.colors`, `topo.colors`, `cm.colors` that make choosing colors easier. Each of these functions takes the number of colors desired as the input and will spit back out a vector of color codes.  Lets try this with `rainbow`:

```{r}
rainbow(5)
```

I can use this vector in my code below:
...
plot(ha$povrate, ha$amtcapita, 
     log="y", yaxt="n", ylim=c(0.1,100000), ylab="",
     xaxt="n", xlim=c(0,60), xlab="Poverty rate",
     pch=21, bg=topo.colors(5)[ha$borough], col="black", cex=log(ha$popn/3000,5))
axis(2,at=c(0.1,1,10,100,1000,10000,100000), las=2,
     labels=paste("$",formatC(c(0.1,1,10,100,1000,10000,100000), 
                                  format="d", big.mark=","), sep=""))
title(ylab="amount per capita", line=5)
...
Now its looking very ... primary. We are almost there but I also want to add a best-fitting line. Normally, I could do this with the `abline` command as discussed in the Canvas section on the OLS regression line. However, that won't work in this case because of the the logarithmic scale on the y-axis. Instead, I can create a sequence of poverty rate values and then based on a model, I can calculate the predicted amount per capita (note that you don't know how to do this yet, so just hang tight). I can then feed those x and y values into a `lines` command to draw a line on my plot. 
