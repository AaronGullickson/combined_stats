...
Slides for this module can be found [here](stat_slides/module5_slides_building_models.html).
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/RaNRPecJIxQ")
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/v1X3VehBUw4")
```
...
These residuals are also sometimes called **error terms**, because the larger they are in absolute value, the worse is our prediction. Take a look at the Figure \@ref(fig:scatter-reside) below which shows the residuals graphically as vertical distances between the actual point and the line. 


```{r scatter-reside, echo=FALSE, fig.cap="Scatterplot with best-fitting line shown in blue and residuals shown in red"}
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/YG8sSaIl6D8")
```
...
```{r scatter-age-violent-line2, echo=TRUE, fig.cap="Use geom_smooth to plot an OLS regression line with or without a confidence interval band"}
ggplot(crimes, aes(x=MedianAge, y=Violent))+
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/sTcdDkWj3Ps")
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/B6Ab63QfXK0")
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/8JTvRLrEsk0")
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/Ow58pdcJzAc")
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/sHicSbJu78U")
```
...
This "controlling" concept is a key point that I will return to below, but first I want to try to think graphically about what this model is doing. In the case of bivariate regression, we thought of fitting a line to a scatterplot in two-dimensional space. We are doing something similar here, but since we now have three variables, our scatterplot is in three dimensions. Figure \@ref(fig:scatter-3d) shows an interactive three-dimensional plot of the three variables. 

```{r scatter-3d, echo=FALSE, fig.cap="Interactive 3d scatterplot of years of education, age, and sexual frequency"}
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/AkRer3eTsAA")
```
...
$$\hat{\texttt{crime}}_i=5137+69(\texttt{pct less hs}_i)-83(\texttt{median age}_i)$$
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/yleq5RTnq-c")
```
...
htmlreg(list(m1, m2, m3),
        ...
        caption="(\\#tab:model-table-example) OLS regression models predicting violent crime rates for US states",
        caption.above=TRUE,
        digits=2,
        star.symbol='&#42;',
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/Nj_cRVzZRQw")
```
...
How would we interpret the slope and intercept for such a model? Figure \@ref(fig:scatter-dichotomous) shows a scatterplot of this relationship.

```{r scatter-dichotomous, echo=FALSE, fig.cap="A scatterplot of the religious indicator variable by sexual frequency. Points are jittered to avoid overplotting. The mean for each group is plotted in red."}
ggplot(sex, aes(x=as.numeric(norelig), y=sexf))+
  geom_smooth(method="lm", se=FALSE)+
  geom_jitter(alpha=0.1, width=0.1, height=5)+
  geom_point(x=0, y=48.33671, color="red", size=3)+
  geom_point(x=1, y=59.84862, color="red", size=3)+
  annotate("label", x=0.1, y=68, label="mean for\nreligious group")+
  annotate("label", x=0.9, y=79, label="mean for\nreligious group")+
  annotate("label", x=0.5, y=30, label="The slope is 11.5,\nthe mean difference\nbetween the groups")+
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/RBgvaGnBLyA")
```
...
Figure \@ref(fig:compare-relig-means) plots these means on a number line to get a visual display of the differences:

```{r compare-relig-means, echo=FALSE, fig.cap="The mean sexual frequency of each group arrayed on a vertical number line. The red lines indicate the distance between each group and the reference category of fundamentalist Protestant."}
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/DllN6ulaqh4")
```
...
Table \@ref(tab:model-relig-sex) below helps to highlight the change in the effects once age is controlled.

```{r model-relig-sex, echo=FALSE, results="asis"}
...
htmlreg(list(m1, m2),
        custom.coef.names=c("Intercept",
                            ...
                            "Age"),
        custom.gof.names = c("R-squared","N"),
        caption="(\\#tab:model-relig-sex) OLS regression models predicting sexual frequency",
        caption.above=TRUE,
        digits=2,
        star.symbol='&#42;',
        custom.note = "%stars. Standard errors in parenthesis. Reference category is fundamentalist Protestant",
        include.adjrs=FALSE, include.rmse=FALSE,
        doctype = FALSE)
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/0SGm6XPLTLA")
```
...
Each line has an intercept and a slope. Notice that the intercepts are different but the slopes are the same. That means we have two parallel lines at different levels. Figure \@ref(fig:plot-nointeractiontwogenre) overlays these two parallel lines on top of a scatterplot of movie runtime by tomato meter for these two genres. 

```{r plot-nointeractiontwogenre, echo=FALSE, fig.cap="Predicted Tomato Meter by runtime for two genres based on an additive OLS regression model. The lines must be parallel."}
model <- lm(TomatoMeter~Runtime+Genre, data=subset(movies, Genre=="Action" | Genre=="Comedy"))
temp <- expand.grid(Runtime=75:185, Genre=c("Action","Comedy"))
temp$TomatoMeter <- predict(model, temp)
ggplot(subset(movies, Genre=="Action" | Genre=="Comedy"),
       aes(x=Runtime, y=TomatoMeter, color=Genre))+
  geom_line(data=temp)+
  geom_point(alpha=0.2)+
  labs(x="runtime in minutes", y="Tomato Meter")+
  theme_bw()
```
...
```{r added-vars-earnings}
earnings$nwhite <- earnings$race!="White"
earnings$college <- as.numeric(earnings$educ)>3
```
...
```{r table-earnings-race-college, echo=FALSE}
...
kable(tab, align=c("r","r","r"),
      caption="Mean wages in dollars per hour by race and education") %>%
  kable_styling(bootstrap_options = c("striped","hover")) %>%
  column_spec(1, bold=TRUE) %>%
  row_spec(3, bold=TRUE, color="white", background="#666") %>%
...
```{r model-additive-earnings-race-college}
...
rownames(tab) <- c("White","Non-white","Difference")
kable(tab, align=c("r","r","r"),
      caption="Predicted wages in dollars per hour by race and education from an additive model") %>%
  kable_styling(bootstrap_options = c("striped","hover")) %>%
  column_spec(1, bold=TRUE) %>%
  row_spec(3, bold=TRUE, color="white", background="#666") %>%
  column_spec(4, bold=TRUE, color="white", background="#666")
...
```{r model-interactive-earnings-race-college}
model <- lm(wages~nwhite*college, data=earnings)
coef(model)
```
...
```{r table-interactive-earnings-race-college, echo=FALSE}
...
rownames(tab) <- c("White","Non-white","Difference")
kable(tab, align=c("r","r","r"),
      caption="Predicted wages in dollars per hour by race and education from an interactive model") %>%
  kable_styling(bootstrap_options = c("striped","hover")) %>%
  column_spec(1, bold=TRUE) %>%
  row_spec(3, bold=TRUE, color="white", background="#666") %>%
  column_spec(4, bold=TRUE, color="white", background="#666")
