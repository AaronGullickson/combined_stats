...
>- Two quantitative variables: The **correlation coefficient** and the **bivariate OLS regression line**
>- Multiple quantitative and/or categorical variables: The **multivariate OLS regression model** (last module of the course)
...
You can easily calculate conditional distributions in *R* using the `prop.table` command on a created table:

```{r}
prop.table(tab,1)
```

Note that I could have calculated the conditional distribution the other way by using `2` rather than `1` in the command. We will return to this later. 
...
## The relationship between age and survival  {.smaller}
<div class="footer">
<body>Sociology 312, Measuring Association: Mean Differences</body>
</div>

```{r fig.width=6, fig.height=4.5, out.width='600px', out.height='450px', dpi=300, fig.align='center', dev.args = list(bg = 'transparent')}
boxplot(age~survival, data=titanic, range=0, ylab="age", col="skyblue",
        main="boxplots of age by survival")
```
...
```{r}
tapply(titanic$age,titanic$survival,mean,na.rm=TRUE)
30.10-29.15
```

Those who died on the Titanic were 0.95 years older than those who survived, on average. 
