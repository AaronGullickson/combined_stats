...
Slides for this module can be found [here](https://aarongullickson.github.io/stat_slides/module4_slides_statistical_inference.html).
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/aOoNQB5kc98")
```
...
Married respondents had sex 2.5 (56.1-53.6) times more per year than never married individuals. We don't want to draw this conclusion just for our sample. Rather, we want to know what the relationship is between marital status and sexual frequency in the US population as a whole. In other words, we want to **infer** from our sample to the population. Figure \@ref(fig:chart-inference) shows this process graphically. 

```{r chart-inference, echo=FALSE, fig.cap="The process of making statistical inferences"}
par(mar=c(0,0,0,0))
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/XGbnpkT1vuM")
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/V_8qkKGmgcI")
```

As an example, lets use some data on a recent class that took this course. I will treat this class of `r sum(!is.na(students$height))` students as a population that I would like to know something about. In this case, I would like to know the mean height of the class. In most cases, the population distribution is unknown but in this case, I know the height of all `r sum(!is.na(students$height))` students because of surveys they all took at the beginning of the term. Figure \@ref(fig:hist-student-height-pop) shows the population distribution of height in the class:

```{r hist-student-height-pop, echo=FALSE, fig.cap="Population distribution of height in a class of students"}
...
What if I were to increase the sample size? I have used the same computer routine to calculate the sampling distribution for samples of size 3, 4, and 5. Figure \@ref(fig:compare-samplingd) shows the results. 


```{r compare-samplingd, echo=FALSE, fig.cap="The sampling distribution of class height for samples of various sizes"}
p1 <- ggplot(data.frame(means2), aes(x=means2))+
  geom_histogram(fill="grey", color="grey20", breaks=seq(from=58,to=76, by=1))+
  labs(title="n=2", x=NULL)+
  xlim(58, 76)+
  theme_bw()+
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank())
p2 <- ggplot(data.frame(means3), aes(x=means3))+
  geom_histogram(fill="grey", color="grey20", breaks=seq(from=58,to=76, by=1))+
  labs(title="n=3", x=NULL)+
  xlim(58, 76)+
  theme_bw()+
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank())
p3 <- ggplot(data.frame(means3), aes(x=means3))+
  geom_histogram(fill="grey", color="grey20", breaks=seq(from=58,to=76, by=1))+
  labs(title="n=4", x=NULL)+
  xlim(58, 76)+
  theme_bw()+
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
        axis.ticks.y=element_blank())
p4 <- ggplot(data.frame(means4), aes(x=means4))+
  geom_histogram(fill="grey", color="grey20", breaks=seq(from=58,to=76, by=1))+
  labs(title="n=5", x=NULL)+
  xlim(58, 76)+
  theme_bw()+
  theme(axis.title.y=element_blank(),
        axis.text.y=element_blank(),
...
# 
# ggplot(temp, aes(x=means, y=..density..))+
#   geom_histogram(data=subset(temp, size=="n=5"), 
#                  breaks=seq(from=58,to=76, by=1))+
#   labs(x="mean height")+
#   xlim(58, 76)+
#   theme_bw()+
...
Clearly, the shape of these distributions is changing. Another way to visualize this would be to draw **density** graphs which basically fit a curve to the histograms. We can then then overlap these density curves on the same graph. Figure \@ref(fig:compare-density) shows this graph. 

```{r compare-density, echo=FALSE, fig.cap="The sampling distribution of class height for samples of various sizes. The vertical red line shows the true population mean."}
...
kable(tab, align=c("l","r","r"),
      caption="Mean and standard deviation of height for the population of students in a class as well as from several sampling distributions of that population") %>%
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/W-TTn-0gIoI")
```
...
```{r normal_dist, echo=FALSE, fig.cap="The Normal Distribution"}
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/CV_IBss49EM")
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/bikmu5nGB9o")
```

As Figure \@ref(fig:normaldist-ci) shows, 95% of all the possible sample means will be within 1.96 standard errors of the true population mean $\mu$.

```{r normaldist-ci, echo=FALSE, fig.cap="95% of all sample means will be within 1.96 standard errors of the true population mean."}
x <- seq(from=-5,to=5,by=0.1)
y <- (1/(sqrt(2*pi)))*exp(-1*(x)^2/(2^2))
...
It follows from the statement above that for 95% of all samples, this interval would contain the true population mean, $\mu$. 

To see how this works graphically, imagine constructing this interval for twenty different samples from the same population. Figure \@ref(fig:conf-intervals) shows the mean and interval for each of these twenty samples, relative to the true population mean. 

```{r conf-intervals, echo=FALSE, fig.cap="Intervals of sample mean plus and minus 1.96 standard errors for 20 different samples, with the true population mean shown by the blue line. Sample means shown by dots."}
par(mar=c(0,0,0,0))
plot(-2,-2, xlim=c(-1,20), ylim=c(-4,4), xaxt="n", yaxt="n", bty="n", xlab="", ylab="")
x <- rnorm(19,0,1)
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/DRnO9h9CdlY")
```
...
We are 95% confident that the  mean age among all US adults is between 48.97 and 50.03 years of age. As you can see, the large sample of nearly 6,000 respondents produces a very tight confidence interval. 

### Calculating the confidence interval for other sample statistics

```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/M_7q99wEifk")
```
...
In order to do this for any of the above sample statistics, we only need to know how to calculate that sample statistic's standard error and the degrees of freedom used to look up the t-statistic for that sample statistic. Table \@ref(tab:cheat-sheet) provides a useful cheat sheet of those formulas:
...
Table: (\#tab:cheat-sheet) Cheat sheet of equations for calculating standard errors and degrees of freedom
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/yMiCBeqz8L0")
```
...
About 27.8% of the respondents in our sample are climate change deniers. What can we conclude about the proportion in the US population? First, lets figure out the t-statistic. We use the same $n-1$ for degrees of freedom:
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/T0h-6fsZXG8")
```
...
```{r}
tab <- tapply(popularity$indegree, popularity$smoker, mean)
tab
mean_diff <- 4.796992 - 4.506699
mean_diff
```
...
```{r}
tapply(popularity$indegree, popularity$smoker, sd)
s1 <- 3.652224
s2 <- 3.901188
...
mean_diff + t_stat*se
```
We are 95% confident that in the population of US adolescents, those who smoke frequently have between 0.03 fewer to 0.61 more friend nominations, on average, than those who do not smoke frequently. Note that because our confidence interval contains both negative and positive value, we cannot be confident about whether smoking is truly associated with having more or less friends. The direction of the relationship between the two variables is uncertain.

### Example with proportion differences

```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/BS5DC752zUQ")
```

Lets continue to use the Add Health data. Do we observe a gender difference in smoking behavior in our sample?

```{r}
prop.table(table(popularity$smoker, popularity$sex), 2)
p_hat_f <- 0.1465106
p_hat_m <- 0.1564593
...
In our sample, about 14.7% of girls were frequent smokers and about 15.6% of boys were frequent smokers. The percentage of boys who smoke is about 1% higher than the percentage of girls. Do we think this moderate difference in the sample is true in the population? 
...
That looks like a lot, but we just have to focus on plugging in the right values in R:
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/YnTTP982LSU")
```
...
```{r}
r <- cor(popularity$pseudoGPA, popularity$indegree)
r
```
...
We are 95% confident that the true correlation coefficient between GPA and friend nominations in the population of US adolescents is between 0.139 and 0.197. While there is some difference in the strength of that relationship, we are pretty confident that the correlation is moderately positive.
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/7KMqOfyLA6s")
```
...
```{r make-believe1, echo=FALSE, fig.cap="A game of make believe, or the sampling distribution for sample proportion of winning Coca-Cola bottle caps assuming the null hypothesis is true"}
x <- seq(from=-0.02,to=0.2,by=0.001)
p <- 0.083
se <- sqrt(p*(1-p)/100)
y <- (1/(se*sqrt(2*pi)))*exp(-1*(x-p)^2/(2*se^2))
par(mar=c(4,0.5,0.5,0.5))
plot(x,y,type="l",xlab="possible sample proportions",yaxt="n", ylab="", cex.main=2)
abline(v=p,lwd=4,col="blue")
text(p,10,"true proportion = 0.083", col="blue", pos=4)
abline(v=0.05,lwd=4,col="red")
text(0.05,10,"our sample      \nproportion = 0.05", col="red", pos=2)
```
...
```{r make-believe2, echo=FALSE, fig.cap="The proportion of all possible sample proportions that are lower than our sample proportion, assuming the null hypothesis is true"}
x2 <- (x-p)/0.028
par(mar=c(4,0.5,0.5,0.5))
plot(x2,y,type="l",xlab="SEs from center",yaxt="n",ylab="")
abline(h=0)
abline(v=-1.18,lwd=4,col="red")
text(-1.18,5,"1.18 standard errors below the mean", col="red", pos=4)
...
```{r make-believe3, echo=FALSE, fig.cap="We have to also consider the possibility of getting a sample proportion as far from the population proportion but in the other direction"}
par(mar=c(4,0.5,0.5,0.5))
plot(x2,y,type="l",xlab="possible sample proportions",yaxt="n",ylab="")
abline(h=0)
polygon(c(x2[1:71],-1.18),c(y[1:71],0), col="grey")
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/jT3oCGtxm0g")
```
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/kE_53FNEyBA")
```
...
I want to look at the difference between Evangelical Protestants and "Other Religions." The mean difference here is:

$$58.32606-60.75311=-2.42705$$

Evangelical Protestants make \$2,427 less than members of other religions, in my sample. 
...
Where $\mu_c$ is the population mean income of evangelical Protestants and $\mu_o$ is the population mean income of members of other religions. In order to figure out how far my sample mean difference of -2.427 is from 0, I need to find the standard error of the mean difference. The formula for this number is:
...
se <- sqrt(51.23396^2/917+56.37886^2/883)
-2.42705/se
``` 

The t-statistic of -0.95 here is not very large. I am only 0.44 standard errors below 0 on the sampling distribution, assuming the null hypothesis is true. Lets go ahead and calculate the p-value for this t-statistic. Remember that I need to put in the negative version of this number to the `pt` command. I also need to use the smaller of the two sample sizes for my degrees of freedom:

```{r}
2*pt(-0.95, 883-1)
```

In a sample of this size, there is an 34% chance of observing a mean income difference of $2,427 or more between evangelical Protestants and members of other religions, just by sampling error, assuming that there is no difference in income in the population. Therefore, I **fail to reject the null hypothesis** that evangelical Protestants and members of other religions make the same income. 

#### Example of proportion differences

```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/aj5tFwkK0MA")
```
...
```{r}
prop.table(table(popularity$smoker, popularity$race),2)
p_w <- 0.20439894
p_b <- 0.05152838
p_diff <- p_w-p_b
p_diff
...
```{r echo=FALSE}
knitr::include_url("https://www.youtube.com/embed/8xbHhe3PgSc")
```

Lets look at the correlation between the parental income of a student and the number of friend nominations they receive. Our null hypothesis will be that there is no relationship between parental income and student popularity in the population of US adolescents. Lets look at the data in our sample:
...
What is the probability of being that far away from zero for a sample of this size?

```{r}
2*pt(-8.334658, n-2)
```

The probability is very small. In a sample of this size, the probability is less than 0.00000001% of observing a correlation coefficient between parental income and friend nominations received of an absolute magnitude of 0.125 or higher when the true correlation is zero in the population. Therefore, we **reject the null hypothesis** and conclude that there is a positive correlation between parental income and popularity among US adolescents.
