...
You can easily fit a LOESS smoother to a scatterplot in `ggplot` by specifying `method="loess"` in the `geom_smooth` function. In fact, this is the default method for `geom_smooth` for datasets smaller than a thousand observations. Figure \@ref(fig:lowess-movies) plots this loess smoother as well as a linear fit for comparison.

```{r lowess-movies, fig.cap="A loess smoother to the data is shown in blue while a linear fit is shown in red.", fig.align="center"}
ggplot(movies, aes(x=TomatoRating, y=BoxOffice))+
  geom_jitter(alpha=0.2)+
...
Move on to a discussion of GAM when data too big for LOESS. 

```{r lowess-wages, fig.cap="Is the relationship between age and hourly wage non-linear?", fig.align="center"}
ggplot(earnings, aes(x=age, y=wages))+
  geom_jitter(alpha=0.01, width=1)+
  geom_smooth(method="lm", color="red", se=FALSE)+
