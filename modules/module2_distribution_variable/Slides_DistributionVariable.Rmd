...
*R* also has a built-in function called `prop.table` that will calculate proportions automatically:

```{r}
prop.table(table(politics$party))
```

Note that I still need to run the `table` command and then wrap `prop.table` around that command.
