...
<div class="footer"> 
<body>Sociology 312, Building Better Models: What is a model?</body>
</div>

Here is the OLS regression line predicting sexual frequency by education:

$$freq\hat{u}ency_i=49.73+0.03*(education_i)$$

The model predicts that a person with zero years of education has sex 49.73 times per year and that a one year increase in education is associated with 0.03 more sexual encounters per year. 

We often refer to this line as a "model." Why?

>- We are measuring the relationship between sexual frequency and education by applying some kind of model to describe this relationship. 
>- Our model is simple. We predict sexual frequency as a **linear function** of years of education. 
...
>- In this section, we will learn a variety of techniques for extending this very basic model of the relationship to produce more realistic, better models of the relationship. 

##  Additional example for this section
<div class="footer"> 
<body>Sociology 312, Building Better Models: What is a model?</body>
</div>

In addition to our usual datasets, we will be examining two published research articles in this section. 

- The first article is "The rewards of authority in the workplace: Do gender and age matter" by Schieman, Schafer, and McIvor. 
- The second article is "Generational Status and Academic Achievement among Latino High School Students: Evaluating the Segmented Assimilation Theory" by Demetra Kalogrides. 
...
>- **Transform** the dependent variable to handle non-linearity. (Advanced)
...
<div class="footer">
<body>Sociology 312, Measuring Association: The OLS Regression Line</body>
</div>
...
<div class="footer"> 
<body>Sociology 312, Building Better Models: The power of controlling for other variables</body>
</div>
...
>- Within each cell, the number on top is the slope. We will learn about he number in parenthesis and the asterisks in the next module.
...
<div style="text-align: center"><img src="images/slides/SchiemanSchaferMcIvorTable1.png"   align="middle" style="width: 350px;"/></div>
...
<div class="footer"> 
<body>Sociology 312, Building Better Models: Using a categorical variable as an independent variable</body>
</div>
...
<div class="footer"> 
<body>Sociology 312, Building Better Models: Interaction terms</body>
</div>
...
$$sup\hat{p}ort_i=80.21-0.30(age_i)-25.06(republican_i)$$

>- The model predicts that, holding party affiliation constant, a one year increase in age is associated with a 0.30 point reduction on the support for gay marriage scale, on average. 
>- The model predicts that, comparing individuals of the same age, Republicans score 25.06 points lower on the support for gay marriage scale than Democrats, on average.
...
>- $sup\hat{p}ort_i=80.21-0.30(age_i)-25.06(0)=80.21-0.30(age_i)$

**Republican:**

>- $sup\hat{p}ort_i=80.21-0.30(age_i)-25.06(1)=(80.21-25.06)-0.30(age_i)=55.15-0.30(age_i)$
...
y.repub <- 55.15-0.30*age
y.dem <- 80.21-0.30*age
...
legend(20,100,legend=c("Republican","Democrat"), lty=1, col=c("red","blue"))
```
...
$$sup\hat{p}ort_i=82.87-0.35(age_i)-32.21(republican_i)+0.14(age_i*republican_i)$$

**Democrats**

$sup\hat{p}ort_i=82.87-0.35(age_i)$

**Republican**

$sup\hat{p}ort_i=(82.87-32.21)+(-0.35+0.14)(age_i)=50.66-0.21(age_i)$
...
```{r echo=FALSE, warning=FALSE, fig.width=8, fig.height=5.5, out.width='800px', out.height='550px', dpi=300, fig.align='center', dev.args = list(bg = 'transparent')}
age <- seq(from=20,to=80,by=1)
y.repub <- 50.66-0.21*age
y.dem <- 82.87-0.35*age
plot(age, y.repub, lwd=2, col="red", type="l", ylim=c(0,100),
     xlab="age", ylab="predicted support", las=1, 
     main="Predicted gay marriage support\nby Republican and Democrats by age", cex.main=1.5)
lines(age, y.dem, lwd=2, col="blue")
...
>- The separate effects of the variables that make up the interaction term are referred to as the **main effects**. In this case, the main effects are -0.35 for age and -32.21 for Republican. 
...
>- **Age Main Effect**: The model predicts that, **among Democrats**, a one year increase in age is associated with a 0.35 point decline in the gay marriage support scale, on average.  
>- **Republican Main Effect**: The model predicts that, **among individuals of zero age**, Republicans have scores on the gay marriage support scale 32.21 points lower than Democrats, on average. 
>- **Interaction Term, version 1**: The model predicts that a one year increase in age **among Republicans** is associated with a 0.14 weaker decline in support for gay marriage than **among Democrats**. (difference in slopes)
>- **Interaction Term, version 2**: The model predicts that a one year increase in age **among Republicans** is associated with a 0.21 decline in the gay marriage support scale, on average. (age slope for indicated category)
>- **Interaction Term, version 3**: The model predicts that each one year increase in age is associated with a 0.14 reduction in the support for gay marriage gap between Republicans and Democrats, on average. (category difference by age)
...
```{r}
politics.reduced$noreligion <- politics.reduced$relig=="Not Religious"
...
---------     ---------   -------------   ----------
Democrat          60.9            83.2          22.3
Republican        36.6            60.6          24.0
Difference       -24.3           -22.6

>- Among Democrats, the non-religious score 21.3 points higher than the religious. 
>- Among Republicans, the non-religious score 24.0 points higher than the religious.
>- Among the religious, Republicans score 24.3 points lower than Democrats. 
>- Among the non-religious, Republicans score 22.6 points lower than Democrats. 
...
$$sup\hat{p}ort_i=60.8-24.0(republican_i)+22.8(norelig_i)$$

Party            Religious         Not Religious   Difference
----------  --------------  --------------------   ----------
Democrat              60.8        60.8+22.8=83.6        22.8
Republican  60.8-24.0=36.8   60.8-24.0+22.8=59.6        22.8
...
$$sup\hat{p}ort_i=60.9-24.3(republican_i)+22.3(norelig_i)+1.6(republican_i)(norelig_i)$$

Party            Religious              Not Religious   Difference
----------  --------------  -------------------------   ----------
Democrat              60.9             60.9+22.3=83.2        22.3
Republican  60.9-24.3=36.6    60.9-24.3+22.3+1.6=60.5        23.9
Difference           -24.3                      -22.7
...
>- **Intercept**: The average gay marriage support score among religious Democrats is 60.9.
>- **Party Main Effect**: Among the religious, Republicans have gay marriage support scores 24.3 points lower than Democrats. 
>- **Religion Main Effect**: Among Democrats, the non-religious have gay marriage support scores 22.3 points higher than the religious.
>- **Interaction term, version 1**: The difference in gay marriage support between the religious and non-religious is 1.6 points greater among Republicans than among Democrats. 
>- **Interaction term, version 2**: The difference in gay marriage support between Republicans and Democrats is 1.6 points smaller among the non-religious than among the religious.  
...
>- The intercept (82.87) and the slope of age (-0.35) apply only to Democrats (the reference group). 
...
>- Republicans: intercept=$82.87-32.21=50.66$, slope=$-0.35+0.14=-0.21$
...
age <- seq(from=20,to=80,by=1)
y.dem <- 82.87-0.35*age
y.repub <- (82.87-32.21)+(-0.35+0.14)*age
y.ind <- (82.86-1.39)+(-0.35-0.10)*age
y.other <- (82.86+8.69)+(-0.35-0.4)*age
plot(age, y.repub, lwd=2, col="red", type="l", ylim=c(0,100),
     xlab="age", ylab="predicted support", las=1, 
...
legend(20,100,legend=c("Republican","Democrat","Independent","Other"), lty=1, col=c("red","blue","purple","green"), ncol=2)
```
...
<div class="footer"  style="top:575px;"> 
<body>Sociology 312, Building Better Models: Transformations</body>
</div>
