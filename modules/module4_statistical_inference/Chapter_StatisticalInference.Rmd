...
Some of these formulas look real nasty. Luckily, we won't spend a lot of time calculating these standard errors by hand. For the regression slope case, where we will spend most of our time in the next module, the SE is already calculated internally by the `lm` command.

### Example with proportions

The course slides cover examples of the confidence interval for mean differences and regression slopes. Here I will show how to calculate the 95% confidence interval for a proportion. As an example, lets use the proportion of respondents who do not believe in anthropogenic climate change. In our politics sample, we get:
...
About 31% of the respondents in our sample are climate change deniers. What can we conclude about the proportion in the US population? First, lets figure out the t-statistic. We use the same $n-1$ for degrees of freedom:
...
The term $hat{p}$ is the standard way to represent the sample proportion, which in this case is 0.306. So, our formula is:

$$\sqrt{\frac{0.3067*(1-0.3067)}{5914}}$$

We can calculate this in *R*:

```{r}
sqrt(0.3067*(1-0.3067)/5914)
```

We now have all the pieces to construct the confidence interval:

```{r}
0.3067+1.96*0.00599621
0.3067-1.96*0.00599621
```

We are 95% confident that the true percentage of climate change deniers in the the US population is between 29.5% and 31.8%. 
