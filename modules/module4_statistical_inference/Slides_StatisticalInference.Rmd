...
```{r}
tapply(politics$age, politics$president, mean)
```

In my ANES sample, Trump voters are older (54.45) than Clinton voters (50.15). Do I think this reflects a real age difference in the population of voters? My null hypothesis is:

$$H_0: \mu_T-\mu_C=0$$

Where $\mu_T$ is the population mean age of Trump voters and $\mu_C$ is the population mean age of Clinton voters. 

The sample mean difference is `r 54.54-50.15` years of age. Is this difference far enough from zero to reject the null hypothesis? 
...
```{r}
tapply(politics$age, politics$president, sd)
table(politics$president)
se <- sqrt(17.32455^2/1364+16.87335^2/1245)
se
```
...
I can now calculate how many standard errors `r 54.54-50.15` is from zero and use that to calculate my p-value. Remember that I use the smaller of the two sample sizes as my degrees of freedom for the t. 
...
The p-value is tiny. Assuming that the true population age difference between Trump and Clinton voters is zero, there is a less than 0.0000000001% chance of observing an age difference of `r 54.54-50.15` years or larger in a sample of this size. Thus, I **reject** the null hypothesis and conclude that Trump voters are older than Clinton voters, on average. 
