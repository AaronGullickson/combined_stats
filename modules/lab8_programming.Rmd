...
```{r readdata}
tracts <- read.csv("resources/tracts_state.csv")
head(tracts)

```

Each observation here is a census tract and we have census tracts identified by state. To see the number of census tracts within each states:
...
The first step is to calculate the proportion of each racial group within each census tract. There are number of ways to do this, but the most straightforward to do this would be to first calculate the total population of each tract and then divide each of the other count variables by this total. We can use the `apply` command to sum up values. The `apply` command will apply a function across either rows or columns. 

```{r, error=TRUE}
alabama_tracts$total <- apply(alabama_tracts[,c("white","latino","black","asian",
                                                "indigenous","other")],1,sum)
head(alabama_tracts)
props <- alabama_tracts[,c("white","latino","black","asian",
                           "indigenous","other")]/alabama_tracts$total
head(props)
```

Its worthwile to check and make sure that all of these values sum up to one across the rows. I can do this with the `apply` command. 
...
In order to calculate Theil's H, I now need the entropy for the state itself and the total population for the state for the denominator of the equation. I can sum up the numbers for the state by again using the `apply` function, but this time sum down the columns rather than across rows:

```{r}
alabama <- apply(alabama_tracts[,c("white","latino","black","asian",
                                   "indigenous","other","total")], 2, sum)
alabama
props_alabama <- alabama[1:6]/alabama[7]
props_alabama
sum(props_alabama)
...
```{r}
1-sum((alabama_tracts$total * alabama_tracts$entropy)/(alabama["total"]*alabama_entropy))
```
...
state_tracts$total <- apply(state_tracts[,c("white","latino","black","asian",
                               "indigenous","other")], 1, sum)
state <- apply(state_tracts[,c("white","latino","black","asian",
                               "indigenous","other","total")], 2, sum)
props <- state_tracts[,c("white","latino","black","asian",
                           "indigenous","other")]/state_tracts$total
props_state <- state[1:6]/state[7]
state_tracts$entropy <- state_tracts$entropy <- apply(props*log(1/props), 1, sum, na.rm=TRUE)
entropy_state <- sum(props_state*log(1/props_state))
theil_h <- 1-sum((state_tracts$total * state_tracts$entropy)/(state["total"]*entropy_state))
theil_h
```
...
In cases like this where you will end up re-using the same code on different but identically structured objects, its often useful to create your own custom function. Custom functions can be created with the following syntax:
...
The function feeds in an object called `state_tracts` which should be the data.frame of tracts for a given stat. The function then runs the same lines of code that I created above. Lets try it out on a couple of states.
...
Without the use of functions and iterative methods, calculating this measure would have been a nightmare. We would have had to copy and paste code fifty different times for each state. Instead, we have been able to reduce the entire process to thirteen lines of code, most of which is just code for the function.

```{r}
calculateTheilH <- function(state_tracts) {
  state_tracts$total <- apply(state_tracts[,c("white","latino","black","asian","indigenous","other")], 1, sum)
  state <- apply(state_tracts[,c("white","latino","black","asian","indigenous","other","total")], 2, sum)
  props <- state_tracts[,c("white","latino","black","asian","indigenous","other")]/state_tracts$total
  props_state <- state[1:6]/state[7]
  state_tracts$entropy <- state_tracts$entropy <- apply(props*log(1/props), 1, sum, na.rm=TRUE)
  entropy_state <- sum(props_state*log(1/props_state))
  theil_h <- 1-sum((state_tracts$total * state_tracts$entropy)/(state["total"]*entropy_state))
...
The result here is also easily *scalable* to larger numbers of higher-level units. It would be easy to re-use this code for metropolitan areas rather than states. I would just need to change a few of the references to states to match the underlying data and I could re-run this code on cities without adding a single new line of code. 