...
For this analysis, we will use the same data extract from the 1980 US Census that we used for the reading/writing lab. I downloaded this extract from [IPUMS](https://usa.ipums.org/usa/).

The data is a sample of individuals in the US. Here is what the data looks like:

```{r read-ipums-cleaning, echo=FALSE}
library(readr)
census <- read_fwf("resources/usa_00074.dat.gz", 
                   col_positions = fwf_positions(start = c(1, 7,15,25,33,37,47,48,51,52,53,
                                                           56,60,63,71,75,83),
                                                 end   = c(4,14,24,27,36,46,47,50,51,
                                                           52,54,58,62,65,74,76,85),
...
All of the data here is recorded as integer values even in cases where the variable is categorical. This is a common approach with many datasets. Part of the cleaning process will be to use the codebook to convert from integer values to proper categorical variables. 
...
One of the first things you should check in your data is how missing values are coded. In many cases, missing values will be coded with a numeric value rather than a proper missing value. If you don't correct this, then all of your statistics will be incorrect. Codebooks that are provided with the data will normally identify the codes used for missing values. Usually, missing values are coded with numeric values that are impossible or well outside the range of the data, so that they can easily be identified. For example, if you look at the [codebook for yrimmig](https://usa.ipums.org/usa-action/variables/YRIMMIG#codes_section) from the IPUMS website, you will see that the zero value is a missing value. Since `yrimmig` is the calendar year of immigration to the United States, zero is a non-sensical value. However, `r formatC(sum(census$yrimmig==0), big.mark=",")` individuals have a value of zero. These are all individuals who should be `NA` on `yrimmig` because they are native-born and thus never had to migrate to the US. If you leave the zeros in, then you will drastically change any calculation you make with this variable. 

In R, we can easily code in missing values by identifying a subset of the variable with a boolean statement and then assigning the special `NA` value to that subset. 

```{r missing-value-bracket-boolean}
summary(census$yrimmig)
census$yrimmig[census$yrimmig==0] <- NA
summary(census$yrimmig)
```

  The mean and median change dramatically once I code in all of the missing values because I had a lot of native-born individuals without a proper value for `yrimmig` who were being reported as zero. 

Notice that there was no comma here for the `col_index` in my brackets. Because I am identifying a specific variable here rather than the full dataset, there is only one dimension (the "rows") to subset along. I refer to this approach to replacing values in a dataset as the **bracket-and-boolean** approach. 

The bracket-and-boolean approach is one of the standard ways to re-code certain values for a variable in R. Another approach that I prefer for its greater flexibility is to use the `ifelse` command. An `ifelse` command takes three arguments. The first argument is a boolean statement. The second argument is what value should be returned if the first argument evaluates to TRUE and the third argument is what value should be returned if the first argument evaluates to FALSE. Here is how I can use the `ifelse` command in this example:

```{r}
census$yrimmig <- ifelse(census$yrimmig==0, NA, census$yrimmig)
```

When `census$yrimmig==0` is TRUE, the `ifelse` command returns `NA` and when `census$yrimmig==0` is false, the `ifelse` command returns the original value of `yrimmig`. This command accomplishes the same thing as the bracket-and-boolean approach above. Where the `ifelse` command really shines is when you can string together multiple `ifelse` commands as I will demonstrate below.
...
A lot of categories are not even showing up in our data. The [reason for these zero categories](https://usa.ipums.org/usa-action/variables/EDUC#comparability_section) is that the set of categories was created to capture all of the ways that educational attainment was recorded across censuses since 1850, but many of the codes are not used in the particular 1980 data that we have.

Many scholars use a parsimonious four-category coding of educational attainment: less than high school diploma, high school diploma, some college including associate's degree, four-year college degree or more. We can collapse our full set of categories into this five-category system using the bracket-and-boolean approach. We start by creating a new variable `edattain_bb` that we assign all missing values. 

```{r create-edattain-bb}
census$edattain_bb <- NA
```

Next we use the bracket-and-boolean approach to replace values in our new variable with character strings. Notice how I use the greater than and less than signs combined with AND statements to create ranges. This is much simpler than having to string together every single possibility

```{r bb-edattain}
census$edattain_bb[census$educd>=2 & census$educd<62] <- "Less than HS"
census$edattain_bb[census$educd>=62 & census$educd<70] <- "HS Diploma"
census$edattain_bb[census$educd>=70 & census$educd<101] <- "Some College"
census$edattain_bb[census$educd>=101 & census$educd<114] <- "College"
```

We now have a variable called `edattain_bb` but it is just a variable of character strings rather than a factor. We can convert to a factor by using the factor command. We will have to specify the levels exactly as I specified them above. 

```{r factor-edattain-bb}
census$edattain_bb <- factor(census$edattain_bb, 
                             levels=c("Less than HS", "HS Diploma",
                                      "Some College","College"))
table(census$educd, census$edattain_bb, exclude=NULL)
```
